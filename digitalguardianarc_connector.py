...
class DigitalGuardianArcConnector(BaseConnector):
    ...
    def _process_html_response(self, response, action_result):
        ...
        message = f"Status Code: {status_code}. Data from server:{self._handle_py_ver_compat_for_input_str(error_text)}"
    ...
    def _process_response(self, response, action_result):
        ...
        try:
            ...
            err = self._get_error_message_from_exception(e)
            exc_tb = sys.exc_info()
            self.save_progress(f"exception_line={exc_tb.tb_lineno} {err}")
            return RetVal(action_result.set_status(phantom.APP_ERROR, (f"Error: {err}")), None)
    ...
    def _make_rest_call(self, endpoint, action_result, method="get", **kwargs):
        ...
        url = "{}/{}".format(self._arc_url.strip("/"), endpoint)
        try:
            self.save_progress(f"Connecting to URL: {url}")
    ...
    def _handle_on_poll(self, param):
        ...
        for entry in export_list:
            try:
                ...
                if comm == -1:
                    comm = 100
                name = ("{alarm_name}-{id}").format(alarm_name=entry["dg_alarm_name"][0:comm], id=entry["dg_guid"])
                if name != oldname:
                    container_id = self.create_container(name, entry)
                    oldname = name
                    if container_id:
                        (artifacts_creation_status, artifacts_creation_msg) = self.create_artifacts(alert=entry, container_id=container_id)
                        if phantom.is_fail(artifacts_creation_status):
                            self.debug_print(f"Error while creating artifacts for container with ID {container_id}. {artifacts_creation_msg}")
                            self._state["first_run"] = False
            except Exception as e:
                err = self._get_error_message_from_exception(e)
                self.debug_print(f"Error occurred while processing export list response from server. {err}")
        return action_result.set_status(phantom.APP_SUCCESS)
    ...
    def get_export(self, action_result):
        ...
        if phantom.is_fail(ret_val):
            return RetVal(action_result.set_status(phantom.APP_ERROR, message), None)
        full_url = "{}/export_profiles/{}/export_and_ack".format(self._arc_url.strip("/"), self._export_profile)
        try:
            request_response = requests.post(url=full_url, headers=self._client_headers, verify=self._verify)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
        ...
        if 200 <= request_status <= 299:
            ...
            message = f"Error from server. Status Code: {request_status} Data from server: {data}"
            return RetVal(action_result.set_status(phantom.APP_ERROR, message), None)
    ...
    def create_container(self, name, items):
        ...
        if not items["dg_alert.dg_detection_source"] == "alert" and items["dg_tags"]:
            ...
            container_dict["start_time"] = ("{time}Z").format(time=datetime.utcfromtimestamp(items["dg_processed_time"] / 1000).isoformat())
            container_dict["source_data_identifier"] = container_dict["name"]
            container_dict["severity"] = self.convert_to_phantom_severity(items["dg_alarm_sev"])
            container_dict["sensitivity"] = self.convert_to_phantom_sensitivity(items["dg_class.dg_name"])
            custom_fields = {"threat type": (items["dg_tags"]), "activity": (items["dg_utype"])}
            container_dict["tags"] = [(f"{x}={custom_fields[x]}") for x in custom_fields if custom_fields[x] is not None]
            ...
            if phantom.is_fail(container_creation_status):
                self.save_progress(
                    ("Error while creating container for alert {alert_name}. {error_message}").format(
                        alert_name=items["dg_alarm_name"], error_message=container_creation_msg
                    )
                )
                return None
            else:
    ...
    def create_artifacts(self, alert, container_id):
        ...
        operation_mapping = {
            ...
            "Netw": ["Alarm", "Process", "Computer", "User", "File", "Network"],
            ...
            "Prin": ["Alarm", "Process", "Computer", "User", "File", "Network"],
            "Othe": ["Alarm"],
        }
        artifacts_mapping = {
            "Alarm": {
            ...
            "Email": {
                "Attachment_File_Name": ("dg_attachments.dg_src_file_name", ["fileName"]),
                ...
                "Email_Recipient_Domain": ("dg_recipients.dg_rec_email_domain", ["domain"]),
            },
            "Network": {
            ...
            "User": {"User_Name": ("dg_user", ["suser"]), "NTDomain": ("ua_dn", [])},
        }
    ...
    def create_dict_hash(self, input_dict):
        if not input_dict:
            ...
            try:
                ...
                self.debug_print(f"Input dictionary is {self._handle_py_ver_compat_for_input_str(input_dict_str)}")
                return
            except Exception as e:
                err = self._get_error_message_from_exception(e)
    ...
    def get_watchlist_id(self, watchListName, action_result):
        ...
        full_url = "{}/watchlists/".format(self._arc_url.strip("/"))
        try:
            r = requests.get(url=full_url, headers=self._client_headers, verify=self._verify)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}"), None)
        try:
            ...
            if 200 <= r.status_code <= 299:
                ...
                message = f"Error from server. Status Code: {r.status_code} Data from server: {data}"
                return RetVal(action_result.set_status(phantom.APP_ERROR, message), list_id)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Unable to process response from the server. {err}"), list_id)

    def _check_watchlist_id(self, watch_list_id, watchlist_entry, action_result):
        full_url = "{}/watchlists/".format(self._arc_url.strip("/"))
        try:
            r = requests.get(url=f"{full_url}{watch_list_id}/values?limit=100000", headers=self._client_headers, verify=self._verify)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}"), None)
        try:
            ...
            err = self._get_error_message_from_exception(e)
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Unable to process response from the server. {err}"), None)

    def get_list_id(self, list_name, list_type, action_result):
        ret_val, message = self.requestApiToken()
        if phantom.is_fail(ret_val):
            return RetVal(action_result.set_status(phantom.APP_ERROR, message), None)
        full_url = "{}/lists/{}".format(self._arc_url.strip("/"), list_type)
        try:
            r = requests.get(url=full_url, headers=self._client_headers, verify=self._verify)
        except Exception as e:
            err = self._get_error_message_from_exception(e)
    ...
    def _add_watchlist_entry(self, param):
        ...
        msg_string = f"{watchlist_entry} to watchlist={watchlist_name}"
        ...
        if watch_list_id:
            ...
            try:
                r = requests.post(
                    url=f"{full_url}{watch_list_id}/values/", data=watch_list_entry_json, headers=self._client_headers, verify=self._verify
                )
            except Exception as e:
                err = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}")
            if 200 <= r.status_code <= 299:
                return action_result.set_status(phantom.APP_SUCCESS, f"Successfully added {msg_string}")
            else:
                return action_result.set_status(phantom.APP_ERROR, f"Failed to add {msg_string}")

        return action_result.set_status(phantom.APP_ERROR, f"Could not find watch_list = {watchlist_name}")
    ...
    def _remove_watchlist_entry(self, param):
        ...
        watchlist_name = self._handle_py_ver_compat_for_input_str(param["watchlist_name"])
        watchlist_entry = self._handle_py_ver_compat_for_input_str(param["watchlist_entry"])
        msg_string = f"{watchlist_entry} from watchlist={watchlist_name}"
        ret_val, watch_list_id = self.get_watchlist_id(watchlist_name, action_result)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        if watch_list_id:
            ...
            if watch_list_value_id:
                ...
                try:
                    r = requests.delete(
                        url=f"{full_url}{watch_list_id}/values/{watch_list_value_id}", headers=self._client_headers, verify=self._verify
                    )
                except Exception as e:
                    err = self._get_error_message_from_exception(e)
                    return action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}")
                if 200 <= r.status_code <= 299:
                    return action_result.set_status(phantom.APP_SUCCESS, f"Successfully removed {msg_string}")
                else:
                    return action_result.set_status(phantom.APP_ERROR, f"Failed to remove {msg_string}")
            else:
                return action_result.set_status(phantom.APP_ERROR, f"Could not find entry {msg_string}")
        else:
            return action_result.set_status(phantom.APP_ERROR, f"Could not find watch_list = {watchlist_name}")
    ...
    def _check_watchlist_entry(self, param):
        ...
        watchlist_name = self._handle_py_ver_compat_for_input_str(param["watchlist_name"])
        watchlist_entry = self._handle_py_ver_compat_for_input_str(param["watchlist_entry"])
        msg_string = f"{watchlist_entry} in watchlist={watchlist_name}"
        ret_val, watch_list_id = self.get_watchlist_id(watchlist_name, action_result)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        if watch_list_id:
            ...
            if phantom.is_fail(ret_val):
                return action_result.get_status()
            if watch_list_value_id:
                return action_result.set_status(phantom.APP_SUCCESS, f"Successfully found {msg_string}")
            else:
                return action_result.set_status(phantom.APP_SUCCESS, f"Failed to find entry {msg_string}")
        else:
            return action_result.set_status(phantom.APP_ERROR, f"Could not find watch_list = {watchlist_name}")
    ...
    def _add_componentlist_entry(self, param):
        ...
        msg_string = f"{componentlist_entry} to componentlist={componentlist_name}"
        ...
        if list_id:
            ...
            full_url = "{}/remediation/lists/".format(self._arc_url.strip("/"))
            try:
                r = requests.put(
                    url=f"{full_url}{list_id}/append", headers=self._client_headers, data=component_list_entry_json, verify=self._verify
                )
            except Exception as e:
                err = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}")
        ...
        return action_result.set_status(phantom.APP_ERROR, f"Could not find component_list = {componentlist_name}")
    ...
    def _remove_componentlist_entry(self, param):
        ...
        componentlist_name = self._handle_py_ver_compat_for_input_str(param["componentlist_name"])
        componentlist_entry = self._handle_py_ver_compat_for_input_str(param["componentlist_entry"])
        msg_string = f"{componentlist_entry} from componentlist={componentlist_name}"
        ret_val, list_id = self.get_list_id(componentlist_name, "component_list", action_result)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        self._client_headers["Content-Type"] = "application/json"
        if list_id:
            ...
            try:
                r = requests.post(
                    url=f"{full_url}{list_id}/delete", headers=self._client_headers, data=component_list_entry_json, verify=self._verify
                )
            except Exception as e:
                err = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}")
    ...
    def _check_componentlist_entry(self, param):
        ...
        componentlist_name = self._handle_py_ver_compat_for_input_str(param["componentlist_name"])
        componentlist_entry = self._handle_py_ver_compat_for_input_str(param["componentlist_entry"])
        msg_string = f"{componentlist_entry} in componentlist={componentlist_name}"
        ret_val, list_id = self.get_list_id(componentlist_name, "component_list", action_result)
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        if list_id:
            full_url = "{}/lists/".format(self._arc_url.strip("/"))
            try:
                r = requests.get(url=f"{full_url}{list_id}/values?limit=100000", headers=self._client_headers, verify=self._verify)
            except Exception as e:
                err = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Error connecting to server. {err}")
            try:
                ...
                err = self._get_error_message_from_exception(e)
                return action_result.set_status(phantom.APP_ERROR, f"Unable to parse JSON response from the server. {err}")
        else:
            return action_result.set_status(phantom.APP_ERROR, f"Could not find component_list = {componentlist_name}")
    ...
    def initialize(self):
        ...
        self.debug_print(f"Action: 'initialize' State: {self._state}")
    ...
    def validateApiToken(self):
        ...
        try:
            api_key_response = requests.post(
                url="{}/as/introspect.oauth2".format(self._auth_url.strip("/")), headers=DG_HEADER_URL, data=payload, verify=self._verify
            )
    ...
    def requestApiToken(self):
        if not self.validateApiToken():
            ...
            try:
                url = "{}/as/token.oauth2".format(self._auth_url.strip("/"))
                api_key_response = requests.post(url=url, headers=DG_HEADER_URL, data=payload, verify=self._verify)
                ...
                err = self._get_error_message_from_exception(e)
                return (phantom.APP_ERROR, f"Error connecting to server. {err}")
            try:
                response_json = api_key_response.json()

                if api_key_response.status_code == 200:
                    self._api_key = response_json["access_token"]
                    self._client_headers.update({"Authorization": f"Bearer {self._api_key}"})
                ...
                err = self._get_error_message_from_exception(e)
                return (phantom.APP_ERROR, f"Unable to process response from the server. {err}")
        else:
            self._client_headers["Authorization"] = f"Bearer {self._api_key}"
            return (phantom.APP_SUCCESS, None)
